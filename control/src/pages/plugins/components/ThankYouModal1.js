...
const ThankYouModal = ({ displayModal, setDisplayModal }) => {
    ...
    return (
        <>
            {displayModal ? (
                <div className={`${css.modalLayer}`} onClick={closeModal} ref={modalRef}>
                    <div className={`col-sm-12 d-flex flex-column }`}>
                        <div className={`p-5 ${css.modalWrap}`}>
                            <div className={`row ${css.modalRow}`}>
                                <div className={`col-sm-12 d-flex flex-column ${css.modalColumn}`}>
                                    <div className={`mx-auto ${css.modalIcon}`}>
                                        <img src={modal_img1} alt='email icon' className={`img-fluid`} />
                                    </div>
                                    <h3 className={`text-center my-4 ${css.modalHeading}`}>
                                        Thank you for subscribing!
                                    </h3>
                                    <p className={`text-center ${css.modalText}`}>
                                        Now, you'll be the first to receive plugin updates <br /> and unique offers from the <br /> Zuri Chat
                                        team.
                                    </p>
                                </div>
                            </div>
