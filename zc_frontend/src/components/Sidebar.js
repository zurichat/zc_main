  ...
  return (
    <div className={styles.container}>
      <div className={styles.zuriLogo}>
        <img src="/assets/zurichatlogo.svg" alt="Zuri Chat logo" />
        <p>ZURI</p>
      </div>
      <div className={styles.orgInfo}>
        <div className={styles.orgName}>
          <p>HNGi8</p>
          <img
            src="/assets/shapekeyboardarrowdown.svg"
            alt="Organisation settings button"
          />
        </div>
        <div className={styles.newMessage}>
          <img src="/assets/newmessage.svg" alt="New message icon" />
        </div>
      </div>
      ...
      <Dropdown title="messages">
        {messagesData &&
          messagesData.messages.map((message, index) => (
            <Fragment key={index}>
              <span>
                <img src={`/assets${message.avatar}`} alt="avatar" />
              </span>
              {message.name}
            </Fragment>
          ))}
