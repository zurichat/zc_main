...
import styles from '../styles/SearchSuggestionBar.module.css'
...
const handleSuggesions=(e)=>{
    ...
    let newArray= sampleSuggestionArray.filter((text)=>(text.toLowerCase().match(value.toLowerCase())));
  ...
  return (
    ...
    <div className={`${styles.searchBar}`}>
       ...
       {(value!=='') && <p className={`position-absolute ${styles.suggested}`}> {`${(suggestions[0])?  suggestions[0] :""}`} </p>}
      <input
        type="text"
        value={value}
      ...
      />
      <img src={cancel} onClick={()=>handleSearchBarClose()} className={`flex-shrink-1 ${styles.searchSvg}`} alt="search" />
    </div>
    {suggestions[0] &&
         (
            <div className={`position-absolute w-100  ${styles.suggestions}`}>
                {
                 suggestions.map((text,i)=><p key={i}> {text}</p>)  
                }
            </div>
        )
    }
