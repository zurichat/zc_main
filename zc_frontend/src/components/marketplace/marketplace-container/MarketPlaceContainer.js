...
import { setPluginId, loadPlugins, fetchPlugins } from '../../../context/marketplace/marketplace.action'
...
const MarketPlaceContainer = () => {
  ...
  const retrievePlugins = async () => {
    ...
    try {
      const response = await axios.get('https://api.zuri.chat/marketplace/plugins')
  ...
  const retrievePlugin = async () => {  
    setisLoading(true)  
    try {
      const response = await axios.get(`https://api.zuri.chat/marketplace/plugins/${marketplace.state.pluginId}`)
      if (response.status === 200 && response.data) {
        const { data } = response.data
        setPlugin(data) 
        setisLoading(false)     
