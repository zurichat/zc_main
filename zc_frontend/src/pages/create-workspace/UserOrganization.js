...
const UserOrganization = ({organizations,user}) => {
    ...
    return (
        ...
        <SelectWorkSpace>
            <p style={{paddingLeft:"10px"}}>Workspaces for <strong style={{fontWeight:"700"}}>{user.email}</strong></p>
            
            { organizations.map((organization) => (
                <OrganizationWrapper key={organization.id}>
                <Image src={organization.logo_url!== "" ? organization.logo_url:ZuriLogo} alt=""/>
                <Organization>
                    <Logo_Members>
                <OrganizationName>{organization.name}</OrganizationName>
                <Members>{organization.no_of_members === 1 ? organization.no_of_members + " member":organization.no_of_members + " members"}</Members>
                </Logo_Members>
                </Organization>
