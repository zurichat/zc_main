...
// Input tag 
const Input = ({ type, placeholder, id, border, label, error, width, value, onChange }) => {
    const err = { message: "Error message Here" }
    return (
        <>
            <BaseLabel border={border}>{label}</BaseLabel>
            <BaseInput value={value} onChange={onChange} type={type} width={width} error placeholder={placeholder} id={id} border={border} />
            {error && <ErrorMessage><img style={{ width: '1.5%', paddingBottom: '0.1rem' }} src={errImg} alt="error icon" />{err.message}</ErrorMessage>}
        </>
    )
}
...
const Toggle = ({ selected, toggleSelected }) => {
    return (
        <div className={`toggle ${selected ? "" : "toggle_disabled"}`} onClick={toggleSelected}>
            <div selected={selected} className={`toggle_ball ${selected ? "" : "toggle_ball_disabled"}`}></div>
        </div>
    )
}
...
const TopNavBar = () => {
    ...
    return (
        <TopNavBarBase >
            <div>
                <img src={zurichatlogo} alt="zuri chat logo" /><LogoName>ZURI</LogoName>
            </div>
            <BaseInput value={search} onChange={(e) => setSearch(e.target.value)} type="text" width={8} error placeholder="Search here" border={"#99999933"} />
            <div>
...
const PluginNavBar = () => {
    ...
    return (
        <PluginNavBarBase >
            <div>
                <PluginName> # </PluginName><PluginName>Enter your plugin name here</PluginName><PluginName> <img src={arrowDown} alt="user profile avatar" />
                </PluginName>
            </div>
            <AllUsers>
                <AllUsersImg src={userAvatar} role="button" alt="user profile avatar" /><AllUsersImg src={userAvatar} role="button" alt="user profile avatar" /><AllUsersImg src={userAvatar} role="button" alt="user profile avatar" /><AllUsersSpan>300</AllUsersSpan>
            </AllUsers>
        </PluginNavBarBase>
    )
}
...
function Test() {
    ...
    return (
        <div>
            ...
            {/* Input tags */}
            <div>
                <Input type="text" width={11} value={word} onChange={(e) => setWord(e.target.value)} placeholder="InActive" border={active} label />
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} width={4} error placeholder="Enter your email here " label="Email" border={error} />
                <Input type="password" width={3} placeholder="Enter your password here" border={inActive} />
            ...
            {/* Toggle */}
            <div>
                <Toggle selected={selected} toggleSelected={() => {
